...
#	Modified by gibbsnich und irosaurus 2012
#
...
sub handler ($)
{
	...
	my ($dir, $stat, $ext) = ($q->path_info() =~
		/^(.*)\/([^\/]+)(\.html|-(day|week|month|year|quarter)\.$imagetype)$/);
	...
	if ($ext eq '.html') {
		do_html($tgt);		
	} elsif ($ext eq '-quarter.' . $imagetype) {
		do_image($tgt, 'quarter');		
...
sub do_html($)
{
	...
	my @quarter = do_image($tgt, 'quarter');
...
EOF
	...
	html_comments($tgt, 'q', @{$quarter[0]}) if $#quarter != -1;
	...
	print "<hr><TABLE><tr><td>";
	html_graph($tgt, 'quarter', 'Quarter Day', $dayavg . ' Minute', \@quarter);
	print "</td><td>";
	html_graph($tgt, 'day', 'Daily', $dayavg . ' Minute', \@day);
	print "</td><td>";
	html_graph($tgt, 'week', 'Weekly', '30 Minute', \@week);
	print "</td></tr><tr><td colspan=3><HR></td></tr><tr><td>";
...
sub html_graph($$$$$)
{
	...
	print "<B>\`$freq\' Graph ($period Average)</B><BR>\n";
...
sub common_args($$$)
{
	...
	$target->{quarter}   = $dir . '/' . $tdir . $name
		. '-quarter.' . $imagetype unless $target->{suppress} =~ /q/;
...
<TABLE BORDER=0 WIDTH=100%>
    <TR>
		<TD COLSPAN=3 ALIGN=CENTER>
			<H1><u>MRTG Monitoring at HOME</u></H1>
		</TD>
	</TR>
    <TR>
EOF
		...
		for my $item (@{$directories{$dir}{target}}) {
			...
			##print "    <TR>\n" unless $odd;
			print <<EOF;
   <TD WIDTH=33% ALIGN=CENTER><A HREF="$item.html">$itemname<BR>
